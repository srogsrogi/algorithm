...
# 경로 탐색 알고리즘 (탄약이 있으면 인접 시 사격, 없으면 전진)
def bfs(grid, start, target, blocked, can_fire):
    rows, cols = len(grid), len(grid[0])
    queue = deque([(start, [])])
    visited = {start}
    ...
    while queue:
        ...
        # 1) 목표가 인접하면: 탄 있으면 사격(F), 없으면 전진(A)
        for d, (dr, dc) in enumerate(DIRS):
            nr, nc = r + dr, c + dc
            if (nr, nc) == target:
                return actions + ([FIRE_CMDS[d]] if can_fire else [MOVE_CMDS[d]])

        # 2) 일반 확장
        for d, (dr, dc) in enumerate(DIRS):
            nr, nc = r + dr, c + dc
            if 0 <= nr < rows and 0 <= nc < cols:
                if grid[nr][nc] not in blocked and (nr, nc) not in visited:
                    visited.add((nr, nc))
...
def has_ammo():
    try:
        me = my_allies.get('M', [])
        normal = int(me[2]) if len(me) >= 3 else 0
        mega   = int(me[3]) if len(me) >= 4 else 0
        return (normal + mega) > 0
    except:
        return False
...
# 최초 경로 탐색
actions = bfs(map_data, start, target, BLOCKED, has_ammo())
...
while game_data is not None:
    ...
    if not actions:
        start, target = find_positions(map_data, START_SYMBOL, TARGET_SYMBOL)
        actions = bfs(map_data, start, target, BLOCKED, has_ammo()) if start and target else []
